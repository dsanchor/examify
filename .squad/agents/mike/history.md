...
**Project**: Examify — Web app for generating exams from PDF sources.

**Agent**: Mike (Backend Dev)  
**Expertise**: Node.js/Express, TypeScript, AI/LLM integration, database design, Docker deployment  
**Owner Area**: aiService, backend architecture, database models, validation, deployment

**Tech Stack**:
- Backend: Node.js, Express, TypeScript
- Frontend: React, TypeScript (managed by Jesse)
- AI: Azure AI Foundry (gpt-5.4-mini), OpenAI SDK
- Data: CosmosDB (partition key: `/id`)
- Deployment: Docker, GitHub Actions (GHCR), Azure Container Apps

**Core Patterns Established**:
1. **AI Extraction**: Verbatim only — no rewriting, no fabrication, no chapter inference
2. **Answer Options**: Preserve actual count from source (2-6), normalize via `padOptions` during exam generation
3. **Chapters**: User-managed labels (no stored content), created/edited/deleted manually after PDF processing
4. **Questions**: Independent of chapters, linked manually via `PUT /api/sources/:id/questions/:questionId/chapter`
5. **Dry Run Exams**: Auto-assembled from all ready sources (129 questions: 120 main + 9 reserve, 4 options each, 120 min timer)
6. **Results**: Reconstructed on-demand from TestSession data (no persistence)
7. **Auth**: Easy Auth headers read via non-blocking middleware, enforcement at proxy level
8. **OneDrive Workaround**: For npm operations, copy package.json → work locally → copy results back

**Current Status**: All backend subsystems operational. Ready for integration testing and deployment validation.

## Recent Work — 2026-04-18

### Verbatim PDF Question Extraction

Finalized the extraction prompt to enforce verbatim Q&A extraction. The AI must extract questions and answers exactly as they appear in source PDFs — no rewriting, rephrasing, or fabrication. Explanations remain AI-generated since they typically don't exist in PDFs.

**Changed**: `EXTRACTION_SYSTEM_PROMPT` and user prompt in `extractFromPdf` (server/src/services/aiService.ts)  
**Impact**: Source fidelity preserved; new uploads will enforce verbatim extraction going forward
